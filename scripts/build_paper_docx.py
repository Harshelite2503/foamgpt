...
if llm is not None and len(llm):
    ...
    exc = json.loads((BENCH_DIR / "llm_agent_exclusions.json").read_text()) if (BENCH_DIR / "llm_agent_exclusions.json").exists() else {}
    S = llm.set_index(["target", "condition"])
    g = lambda t, c, k: S.loc[(t, c)][k]  # noqa: E731
    P(f"Scoring excluded {exc.get('control_rows', 0)} unfilled-matrix control rows and {exc.get('density_visible_in_sibling', 0)} density tasks whose record also appeared as a strength/modulus task in the same batch (its density was visible as an input there). Three observations follow. First, zero-shot, the LLM is a far better predictor of mechanical properties than the cross-laboratory ML baselines: median absolute percentage error is {g('strength_mpa','zero_shot','median_ape_pct'):.0f}% for strength and {g('modulus_mpa','zero_shot','median_ape_pct'):.0f}% for modulus (log-R² {g('strength_mpa','zero_shot','r2_log'):.2f} and {g('modulus_mpa','zero_shot','r2_log'):.2f}), against tree-ensemble MAPE above 85% and log-R² ≤ 0.18. The LLM brings prior knowledge of matrix classes, microballoon grades and typical property ranges that 140 training rows cannot supply; for density its zero-shot error ({g('measured_density_g_cc','zero_shot','median_ape_pct'):.0f}% median) is comparable to the random forest, because both are essentially applying the rule of mixtures. Second, retrieval helps: five records from other papers reduce median error to {g('strength_mpa','rag_5','median_ape_pct'):.0f}% (strength), {g('modulus_mpa','rag_5','median_ape_pct'):.0f}% (modulus) and {g('measured_density_g_cc','rag_5','median_ape_pct'):.1f}% (density), and raise log-R² to {g('strength_mpa','rag_5','r2_log'):.2f} / {g('modulus_mpa','rag_5','r2_log'):.2f} / {g('measured_density_g_cc','rag_5','r2_log'):.2f}. Third, calibration is imperfect but not wildly so: empirical coverage of the stated 90% intervals is {g('strength_mpa','zero_shot','coverage_90'):.0%} (strength) and {g('modulus_mpa','zero_shot','coverage_90'):.0%} (modulus) zero-shot — over-confident — and improves to {g('strength_mpa','rag_5','coverage_90'):.0%} and {g('modulus_mpa','rag_5','coverage_90'):.0%} with retrieval. Mean errors remain large ({g('strength_mpa','zero_shot','mape_pct'):.0f}% zero-shot strength) because of a heavy tail: under-specified records (no volume fraction or density), split-Hopkinson-bar moduli, and metal-matrix rows where 'strength' may denote yield, peak or plateau stress.")
    P("Caveats specific to this benchmark: (i) the retrieval pool falls back to the nearest volume fraction regardless of matrix when a matrix/particle class is sparse, so several aluminium targets received polymer-foam references — retrieval was therefore a distractor for some tasks and the RAG gain is a lower bound for a class-aware retriever; (ii) 'other papers' can be other theses from the same research group with identical materials, a same-laboratory channel that paper-level splitting does not close; (iii) a few task pairs share byte-identical descriptors with different measured values, bounding achievable error; (iv) the model was prompted through Claude Code with file access restricted to the task text, but it is the same model family that performed extraction.")
else:
